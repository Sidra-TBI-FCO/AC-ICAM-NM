...
# Analysis
df_plot = df
df_plot$var02_ICR_cluster = factor(df_plot$var02_ICR_cluster, levels = c("ICR Low", "ICR Medium", "ICR High"))

plot = ggplot(df_plot, aes(x = var02_ICR_cluster, y = get(var_of_interest))) +
  geom_boxplot(outlier.shape = NA, aes(fill= var02_ICR_cluster)) +
...
png(paste0("./Figures/Multiomics_Survival_Model/004_Var_of_interest_plots/", var_of_interest, 
           "_by_ICR_cluster_in_ACICAM", subset, ".png"), res = 600, width = 3, height = 4, units = "in")
plot(plot)
dev.off()
