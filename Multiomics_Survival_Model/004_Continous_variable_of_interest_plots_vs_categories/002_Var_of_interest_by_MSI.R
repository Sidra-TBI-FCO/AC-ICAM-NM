...
# Analysis
df_plot = df
df_plot$var19_MSI_MANTIS = factor(df_plot$var19_MSI_MANTIS, levels = c("MSS", "MSI-H"))

plot = ggplot(df_plot, aes(x = var19_MSI_MANTIS, y = get(var_of_interest))) +
  geom_boxplot(outlier.shape = NA, aes(fill= var19_MSI_MANTIS)) +
...
png(paste0("./Figures/Multiomics_Survival_Model/004_Var_of_interest_plots/", var_of_interest, 
           "_by_MSI_Status_in_ACICAM", subset, ".png"), res = 600, width = 3, height = 4, units = "in")
plot(plot)
dev.off()

plot = ggplot(df_plot, aes(x = var12_Hypermutation_status, y = get(var_of_interest))) +
  geom_boxplot(outlier.shape = NA, aes(fill= var12_Hypermutation_status)) +
...
png(paste0("./Figures/Multiomics_Survival_Model/004_Var_of_interest_plots/", var_of_interest, 
           "_by_Hypermutation_status_in_ACICAM", subset, ".png"), res = 600, width = 3, height = 4, units = "in")
plot(plot)
dev.off()
