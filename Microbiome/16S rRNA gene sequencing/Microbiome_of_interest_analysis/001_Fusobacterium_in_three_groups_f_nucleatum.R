...
load("./Analysis/Exploration_reviewers/Microbiome/tumor_only_246/fusobacterium_analysis/clinical_data_with_fusobacterium_grroups.Rdata")
...
results = data.frame(Name = "Fusobacterium_3_groups", p_val = NA, HR = NA, CI_lower = NA, CI_upper = NA)

# add microbiome variable
Merged_dataset$Microbiome_var = Merged_dataset$group
...
Cal.Surv = TS.Surv
mHR = coxph(formula = msurv ~ Cal.Surv$Microbiome_var,data = Cal.Surv, subset = Cal.Surv$Microbiome_var %in% c("Negative", "High"))
...
results$p_val[which(results$Name == "Fusobacterium_3_groups")] = p_val
results$HR[which(results$Name == "Fusobacterium_3_groups")] = HR 
results$CI_lower[which(results$Name == "Fusobacterium_3_groups")] = CI_lower
results$CI_upper[which(results$Name == "Fusobacterium_3_groups")] = CI_upper
...
ggsurv_plot = ggsurvplot(mfit,
                         ...
                         censor.size = 1.5,
                         pval = TRUE,
                         palette = c("darkblue", "purple", "orange")
) 


dir.create("./Figures/Microbiome/Microbiome_Of_Interest_Plots/MOI_07_Kaplan_Meier_OS_by_WGS_Species", showWarnings = FALSE)
png(paste0("./Figures/Microbiome/Microbiome_Of_Interest_Plots/MOI_07_Kaplan_Meier_OS_by_WGS_Species/KM_", MSI, "_",
           "Kaplan_Meier_Fusobacterium_3_groups_", Survival ,"_by_zero_cutoff.png"),
