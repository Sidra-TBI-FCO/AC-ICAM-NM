...
wgs = read.xlsx("./Analysis/Exploration_reviewers/Microbiome/tumor_only_246/WGS_validation/Microbiome_WGS_updated_167.xlsx", sheet = 2)
...
rownames(wgs) = wgs$Species
wgs$Species = NULL

if (cohort == "overlapping") {
  pcr = pcr[!is.na(pcr$PCR),]
  pcr = pcr[!is.na(pcr$`WGS.Species.=.Bromii`),]
  wgs = wgs[,which(colnames(wgs) %in% pcr$Sample)]
}

clinical_data = clinical_data[which(clinical_data$ID %in% colnames(wgs)),]

wgs = t(wgs)
wgs = as.data.frame(wgs)

clinical_data$bromi = wgs$s__Ruminococcus_bromii[match(clinical_data$ID, rownames(wgs))]

m.bromi = median(clinical_data$bromi)

clinical_data$group = NA
clinical_data$group[which(clinical_data$bromi <= m.bromi)] = "Negative"
clinical_data$group[which(clinical_data$bromi > m.bromi)] = "Positive"
...
results_df = data.frame(Survival = "WGS-Ruminococcus bromii", HR = NA, p_val = NA, CI_lower = NA, CI_upper = NA, p_val_logrank = NA)
...
save(results_df, file = paste0("./Analysis/Exploration_reviewers/Microbiome/tumor_only_246/021_HR_tables_Median_and_continuous/HR_WGS_",survival,"_",cohort,"_samples_categorical.Rdata"))
