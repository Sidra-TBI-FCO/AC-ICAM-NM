...
## Survival Analysis with all continuous variable in separate script:
...
for (i in 1:ncol(immune_sig_df)){
  ...
  TS.Alive = clinical_data[clinical_data$OS.Status == "Alive", c("OS.Status", "OS.Time", Group.of.interest)]
  colnames(TS.Alive) = c("Status","Time", Group.of.interest)
  TS.Alive$Time = as.numeric(as.character(TS.Alive$Time))
  TS.Alive$Time[TS.Alive$Time > Y] = Y
  
  TS.Dead = clinical_data[clinical_data$OS.Status == "Dead", c("OS.Status", "OS.Time", Group.of.interest)]
...
save(HR_table, file = paste0("./Analysis/Trimmed_p/013_Immune_signatures_survival/013_Aug_2022_OS_HR_table_immune_signatures_",
                            exclude_adjuvant, "_cutoff_", Surv.cutoff.years,".Rdata"))

write.csv(HR_table, file = "./Analysis/Trimmed_p/013_Immune_signatures_survival/013_Aug_2022_OS_HR_table_immune_signatures__cutoff_20.csv",
          row.names = TRUE)
