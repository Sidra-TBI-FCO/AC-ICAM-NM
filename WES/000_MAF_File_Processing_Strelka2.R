...
# Load data
merged_txt = read.csv("./WES_HPC/SDR_WH_WES_CGL_JSREP_DAVIDE/8_merge_maf_files/Strelka2/strelka2_all_samples.maf", sep = "\t")
head.txt = read.table("./WES_HPC/SDR_WH_WES_CGL_JSREP_DAVIDE/8_merge_maf_files/Strelka2/header_strelka2_all_samples.txt", sep = "\t", stringsAsFactors = FALSE)
...
# Correction batch 272 is a Liver Metastasis, no primary tumor (double checked)
MAF_df$Tumor_Sample_Barcode = gsub("WES_COAD_LUMC_SIDRA_272T_sorted", "WES_COAD_LUMC_SIDRA_272LM_sorted", MAF_df$Tumor_Sample_Barcode)
...
MAF_df$Tumor_Sample_Barcode = gsub("_sorted", ".sorted", MAF_df$Tumor_Sample_Barcode)
MAF_df$Matched_Norm_Sample_Barcode = gsub("_sorted", ".sorted", MAF_df$Matched_Norm_Sample_Barcode)
...
save(strelka2_MAF_df, file = paste0("./Processed_Data/WES/MAF/MAF_strelka2_all_version_", version, ".Rdata"))
