...
# Plot
plot = ggplot(df, aes(x = Histology_simple, y = ICRscore, fill = Histology_simple)) +
  ...
  xlab("") +
  ylab("") +
  #stat_compare_means(comparisons = list(c("HR_MUT", "WT")), method = "t.test") +
  theme_bw() +
  ylim(4, 11) +
  ylab("ICR score") +
...
svg("./Figures/WES/029_ICRscore_by_mucinous_subtype/v2Boxplot_ICRscore_by_Mucinous_and_others_in_hypermutated.svg",
    #width = 2.4, height = 3, pointsize = 12)
    width = 3.5, height = 3, pointsize = 12)
plot(plot)
dev.off()

test = t.test(df$ICRscore[which(df$Histology_simple == "Others")], df$ICRscore[which(df$Histology_simple == "Mucinous")])
test$p.value

df_muc = df[which(df$Histology_simple == "Mucinous"),]
test = t.test(df_muc$ICRscore[which(df_muc$HR_status == "HR_MUT")], df_muc$ICRscore[which(df_muc$HR_status == "WT")])
test$p.value

df_oth = df[which(df$Histology_simple == "Others"),]
test = t.test(df_oth$ICRscore[which(df_oth$HR_status == "HR_MUT")], df_oth$ICRscore[which(df_oth$HR_status == "WT")])
test$p.value
