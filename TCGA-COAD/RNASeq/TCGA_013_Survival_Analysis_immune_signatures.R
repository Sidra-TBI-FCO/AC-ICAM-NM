...
for (i in 1:ncol(immune_sig_df)){
  ...
  TS.Alive = clinical_data[clinical_data$OS == "0", c("OS", "OS.time", Group.of.interest)]
  colnames(TS.Alive) = c("Status","Time", Group.of.interest)
  TS.Alive$Time = as.numeric(as.character(TS.Alive$Time))
  TS.Alive$Time[TS.Alive$Time > Y] = Y
  
  TS.Dead = clinical_data[clinical_data$OS == "1", c("OS", "OS.time", Group.of.interest)]
...
save(HR_table, file = paste0("./Analysis/013_Immune_signatures_survival/013_", geneset, "_OS_HR_table_",
