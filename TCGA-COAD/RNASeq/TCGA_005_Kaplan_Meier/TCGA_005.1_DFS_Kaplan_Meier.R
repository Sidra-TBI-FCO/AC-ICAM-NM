...
TS.Alive = Merged_dataset[Merged_dataset$PFI == "0", c("PFI", "PFI.time", Group.of.interest, "ICRscore")]
colnames(TS.Alive) = c("Status","Time", Group.of.interest, "ICRscore")
TS.Alive$Time = as.numeric(as.character(TS.Alive$Time))
TS.Alive$Time[TS.Alive$Time > Y] = Y

TS.Dead = Merged_dataset[Merged_dataset$PFI == "1", c("PFI", "PFI.time", Group.of.interest, "ICRscore")]
...
Cal.Surv[,Group.of.interest] = as.factor(Cal.Surv[,Group.of.interest])
mHR = coxph(formula = msurv ~ Cal.Surv[,Group.of.interest],data = Cal.Surv, subset = Cal.Surv[, Group.of.interest] %in% c("ICR High", "ICR Low"))
...
# plots
png(paste0("./Figures/Kaplan Meier Plots/", download.method, "_", exclude_stage_IV, "_", CMS,"_PFI_", Group.of.interest,"_Surv_cutoff_years_",Surv.cutoff.years,".png"),res=600,height=6,width=8,unit="in")  # set filename
ggkm(mfit,
     timeby=12,
